    ...
    for (i = 0; i < 6; i++)
        if (player->buildings[i].lvl != 0) {
            ...
            printf("\n%-2d\t%-15s\t%-15d\t%-15d\t%-15s\t%-15s\t%5s%c\n", counter,
                   name_building[player->buildings[i].building_type],
...
inline void printboard_delete_up_resource(user *player) {
    ...
    for (i = 0; i < 6; i++)
        ...
        if (player->resources_generators[i].lvl != 0 && player->resources_generators[i].status == DEFAULT) {
            resource_requirements(player->resources_generators[i].resource, player->resources_generators[i].lvl - 1,
                                  resource_req);
            printf("\n  %d\t%s\t%d\t%d\t%d\t%d\t%d\n", counter,
                   name_resource[(int) player->resources_generators[i].resource],
                   player->resources_generators[i].lvl, player->resources_generators[i].collect_rate, resource_req[0],
                   resource_req[1], resource_req[2]);
            counter++;
        }
    ...
    for (i = 0; i < 6; i++)
        ...
        if (player->buildings[i].lvl != 0 && player->buildings[i].status == DEFAULT) {
            ...
            printf("\n  %d\t%s\t%d\t%d\t%d\t%d\t%d\n", counter, name_building[player->buildings[i].building_type],
                   player->buildings[i].lvl, player->buildings[i].storage, building_req[0], building_req[1],
                   building_req[2]);
            counter++;
        }
...
inline void print_storage(user *player, int capacity[]) {
    printf("\nstorage resources: wood:%d/%d\tfood:%d/%d\tstone:%d/%d", player->resources.wood_count, capacity[0],
           player->resources.food_count, capacity[1], player->resources.stone_count, capacity[2]);
}
...
inline void print_power_of_village(user *player) {
    ...
    attack = player->soldiers_count.archer * 15 + player->soldiers_count.warrior * 10 + player->soldiers_count.rider *
             20;
...
print_soldiers_creating(user *player, int state[]) {
    ...
    for (i = 0; i < 18; i++) {
        if (player->soldiers[i].number_soldiers != 0) {
            ...
            printf("_\n%d\t%s\t%d\t%s\t%s\t", counter, name_soldiers[(int) player->soldiers[i].soldier],
                   player->soldiers[i].number_soldiers, change_time, finishing_time);
        }
    }
}
...
inline void print_add_soldier(user *player, int state[]) {
    printf("choose your barrack building or return back:\n");
    int counter = 1;

    if (state[0] != -1) {
        printf(
            "\n%d_ barrack building  level =%d the rate of creating soldiers per minute:%d,number of bulidable soldiers now:%d",
            counter, player->buildings[state[0]].lvl, player->buildings[state[0]].lvl + 1,
            player->buildings[state[0]].storage);
        counter++;
    }
    if (state[1] != -1) {
        printf(
            "\n%d_barrack building  level =%d the rate of creating soldiers per minute:%d,number of bulidable soldiers now:%d",
            counter, player->buildings[state[1]].lvl, player->buildings[state[1]].lvl + 1,
            player->buildings[state[1]].storage);
    ...
    if (state[2] != -1) {
        printf(
            "\n%d_barrack building  level =%d the rate of creating soldiers per minute:%d,number of bulidable soldiers now:%d",
            counter, player->buildings[state[2]].lvl, player->buildings[state[2]].lvl + 1,
            player->buildings[state[2]].storage);
