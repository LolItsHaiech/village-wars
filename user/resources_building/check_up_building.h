...
inline void check_up_building(user *player, enum Buildings building_type, int state, int level) {
    ...
    if (level == 5)
        ...
        switch (building_type) {
            case FOOD_STORAGE:
                if (player->resources.wood_count < food_storage_reqs[level - 1].wood || player->resources.stone_count <
                    food_storage_reqs[level - 1].stone)
                    result = 0;
            ...
            case STONE_STORAGE:
                if (player->resources.wood_count < stone_storage_reqs[level - 1].wood || player->resources.stone_count <
                    stone_storage_reqs[level - 1].stone)
                    result = 0;
