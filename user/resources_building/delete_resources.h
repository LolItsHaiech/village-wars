   ...
   if(1<=choice&&choice<=number_resources)
    {
     ...
     printf("are you sure to delete this resource %s level %d ?\n1.yes\n2.no",name_resource[player->resources_generators[state[choice-1]].resource],player->resources_generators[state[choice-1]].lvl);
     scanf("%d",&exept_choice);
     while(exept_choice!=1&&exept_choice!=2)
         {printf("your choice is incorect try again");
         system("cls");

      printf("are you sure to delete this resource %s level %d ?\n1.yes\n2.no",name_resource[(int)player->resources_generators[state[choice-1]].resource],player->resources_generators[state[choice-1]].lvl);
