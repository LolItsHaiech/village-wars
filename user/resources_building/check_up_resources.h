...
void check_up_resource(user*player,enum Resouce resource ,int state,int level)
{
  ...
  if(level==5)
  ...
  else
    {
      switch(resource)
        {
          case FOOD:
             if(player->resources.wood_count<food_reqs[level-1].wood||player->resources.stone_count<food_reqs[level-1].stone)
                result=0;
          ...
          case WOOD:
             if(player->resources.wood_count<wood_reqs[level-1].wood||player->resources.stone_count<wood_reqs[level-1].stone)
               result=0;
           ...
           case STONE:
             if(player->resources.wood_count<stone_reqs[level-1].wood||player->resources.stone_count<stone_reqs[level-1].stone)
               result=0;
      ...
      if(result)
        {
         ...
         collect_source(player,state,resource,level+1); player->resources_generators[state].status=upgrading;
